with required_actions as (
  select
    case 
      when (action = 'SIGN_AGREEMENTS' and corporations.id is not null) then 'SIGN_AGREEMENTS (corporation)'
      when (action = 'SIGN_AGREEMENTS' and people.id is null and corporations.id is null) then 'SIGN_AGREEMENTS (reg firm + interested party)'
  ...
  left join pantheon.so_clients.corporations on corporations.id = account_required_actions.target_id
  ...
  and accounts.type in ('CA_CORPORATE')
)
select 
